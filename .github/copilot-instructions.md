...
- Never commit private keys, PFX/PKCS#12, JKS, passwords, tokens, or decrypted secret material. Where Workspace signing is required, the canonical GitHub Actions contract is `KYRON_CODESIGN_PFX_BASE64` + `KYRON_CODESIGN_PASSWORD`, and the certificate must match `security/signing/canonical-signer.json`.
