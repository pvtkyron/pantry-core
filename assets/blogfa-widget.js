(()=>{
if(window.__REV_BLOGFA_WIDGET__)return;
window.__REV_BLOGFA_WIDGET__=1;
if(window.__REV_BLOGFA_WIDGET_V3__)return;
const OWNER='pvtkyron',REPO='pantry-core',FILE='assets/blogfa-widget-v3.js';
const src='https://cdn.jsdelivr.net/gh/'+OWNER+'/'+REPO+'@main/'+FILE+'?legacy=v1&rev='+Date.now();
const s=document.createElement('script');
s.async=true;
s.src=src;
s.dataset.revLegacyWidget='v1';
s.onerror=()=>console.error('[ProjectRev 旧ウィジェット] 日本語版v3ランタイムを読み込めませんでした');
document.head.appendChild(s);
})();
